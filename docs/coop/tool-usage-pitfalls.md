...
## P-008 · dispatch headless do opencode silenciosamente sem provider funcional configurado

**Data:** 2026-08-06 (DEVX-049/DEVX-050)
**Sintoma:** `worker-start --agent opencode` (ou qualquer `opencode run`/`opencode serve` disparado headless por este app) falha a inferência real com erros do tipo "You need positive balance to do inference" ou similar, mesmo com o `opencode serve` respondendo saudável (`GET /global/health` → `healthy: true`) e a sessão sendo criada normalmente — o dispatch "funciona" mas a LLM por trás nunca responde de verdade.
**Causa raiz:** `~/.config/opencode/opencode.jsonc` (config nativo do opencode, **separado** do `crush.json`/`.env`/Headroom que só valem para o Crush) não tinha nenhum provider real configurado — opencode caía num provider/modelo default embutido sem saldo. A lista real de providers/API-keys do ambiente vive em três arquivos: `C:\Users\israe\AppData\Local\crush\crush.json`, `C:\Dev2026\Docs\.env` e `C:\Dev2026\Docs\scripts\headroom-proxies.mjs`, mas esses arquivos configuram os providers para passar pelo proxy local do Headroom (portas 127.0.0.1:87xx) — que só roda quando o Crush está ativo, sem garantia nenhuma de estar de pé quando o CoopCode despacha o opencode headless.
**Solução aplicada:** `~/.config/opencode/opencode.jsonc` foi populado com os mesmos 10 providers, apontando **direto pra API real de cada um** (bypass do proxy Headroom), usando `@ai-sdk/openai-compatible` + `baseURL`/`apiKey` literais (não `{env:VAR}` — não há garantia de que a variável exista no processo que spawna o opencode headlessly, é o mesmo tipo de bug que já causou P-007/o gap do PATH). `"model": "opencode-go/deepseek-v4-flash"` como default. Verificado ao vivo: `opencode-go/deepseek-v4-flash` responde de verdade; `deepseek/deepseek-v4-flash` (API direta da DeepSeek) falha por saldo insuficiente — confirmando que múltiplos providers configurados, não um só, é necessário.
**Evidência:** `opencode run "reply with exactly the word: pong" --model opencode-go/deepseek-v4-flash` → `pong` real. `opencode run ... --model deepseek/deepseek-v4-flash` → `Error: Insufficient Balance`. Reprodução completa de uma recusa real de ação negada capturada com `opencode-go/deepseek-v4-flash` + perfil restrito (`bash: deny`), registrada em `docs/coop/tasks/DEVX-050.md`'s "Dispatcher note (2026-08-06)".
**Como prevenir recorrência (padrão daqui pra frente):** qualquer spec/dispatch que envolva `opencode run`/`opencode serve` headless deve (1) especificar explicitamente `-m`/`--model` com **`opencode-go/deepseek-v4-flash` como preferência primária**, nunca depender do default implícito do config; (2) tratar uma falha de provider (saldo/auth/rede reportado pelo próprio opencode) como um erro reportável nomeando qual provider falhou, nunca como um erro genérico de conexão nem como uma tentativa silenciosa de outro provider sem registrar; (3) manter uma lista curta de fallbacks ordenados (ex.: `deepseek/deepseek-v4-flash`, `opencode-zen/deepseek-v4-flash-free`) documentada no próprio código de dispatch, não só na cabeça de quem escreveu a spec.
**Limites:** o config em `~/.config/opencode/opencode.jsonc` é local a esta máquina, fora de qualquer repositório — não é versionado nem se propaga para outra máquina/CI automaticamente; se o ambiente de dispatch mudar de máquina, este arquivo precisa ser recriado a partir dos mesmos três arquivos-fonte. As chaves de API estão em texto plano nesse arquivo, mesmo padrão já usado em `crush.json`/`.env` neste ambiente.
