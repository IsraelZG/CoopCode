...
export const ORCHESTRATION_WORKER_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    notes: [
      ...
      '--agent opencode dispatches headlessly (opencode serve + run --attach), never through the crashing interactive TUI; it requires an existing worktree and rejects --terminal and --on.',
      '--opencode-agent-profile <name> with --opencode-agent-permissions <read,glob,grep> builds a restricted profile via opencode agent create --mode subagent --permissions and dispatches through it; both flags are opencode-only.',
