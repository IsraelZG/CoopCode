[CmdletBinding()]
param(
    [string]$AppDir = (Join-Path $PSScriptRoot '..\..\dist\win-arm64-unpacked'),
    [string]$Installer = (Join-Path $PSScriptRoot '..\..\dist\orca-windows-setup.exe')
)
...
function Test-ManagedAnyCpu {
    ...
    try {
        ...
        $flags = if ($null -eq $corHeader) { 0 } else { [int]$corHeader.Flags }
        $isIlOnly = ($flags -band 0x1) -ne 0
        $requires32Bit = ($flags -band 0x2) -ne 0
        if ($isIlOnly -and -not $requires32Bit) {
            $script:passed++
            Write-Host "[PASS] $Label - managed AnyCPU"
        }
        else {
            $script:failed++
            Write-Host ('[FAIL] {0} - CLR flags 0x{1:X}' -f $Label, $flags)
        }
    }
    finally {
        $reader.Dispose()
...
Test-ManagedAnyCpu 'Orca CLI launcher' (Join-Path $resources 'bin\orca.exe')
...
if (-not (Test-Path -LiteralPath $Installer -PathType Leaf)) {
    ...
    Write-Host "Installer SHA256: $(Get-Sha256 -Path $Installer)"
}

$appItem = Get-Item -LiteralPath (Join-Path $AppDir 'Orca.exe')
Write-Host "Orca.exe bytes: $($appItem.Length)"
Write-Host "Orca.exe SHA256: $(Get-Sha256 -Path $appItem.FullName)"
Write-Host ''
